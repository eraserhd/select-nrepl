(ns ^:no-doc select-nrepl.rewrite-clj.zip.base
  (:refer-clojure :exclude [print])
  (:require [select-nrepl.rewrite-clj
             [node :as node]
             [parser :as p]]
            [select-nrepl.rewrite-clj.zip.whitespace :as ws]
...
(defn edn*
  "Create zipper over the given Clojure/EDN node.
   ...
   return the current row/column using `select-nrepl.rewrite-clj.zip/position`. (Note that

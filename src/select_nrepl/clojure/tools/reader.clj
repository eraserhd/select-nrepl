...
(ns ^{:doc "A clojure reader in clojure"
  ...
  (:require [select-nrepl.clojure.tools.reader.reader-types :refer
            ...
            [select-nrepl.clojure.tools.reader.impl.utils :refer :all] ;; [char ex-info? whitespace? numeric? desugar-meta]
            [select-nrepl.clojure.tools.reader.impl.errors :as err]
            [select-nrepl.clojure.tools.reader.impl.commons :refer :all]
            [select-nrepl.clojure.tools.reader.default-data-readers :as data-readers])
  (:import (clojure.lang PersistentHashSet IMeta
                         RT Symbol Reflector Var IObj
                         PersistentVector IRecord Namespace)
           select_nrepl.clojure.tools.reader.reader_types.SourceLoggingPushbackReader
...
(def ^:dynamic *read-eval*
   ...
   ***WARNING***
   ...
   used with untrusted sources. See also: select-nrepl.clojure.tools.reader.edn/read.
...
(def ^:dynamic *data-readers*
  "Map from reader tag symbols to data reader Vars.
   ...
   Default reader tags are defined in select-nrepl.clojure.tools.reader/default-data-readers
...
(defn read
   ...
   ***WARNING***
   ...
   To read data structures only, use select-nrepl.clojure.tools.reader.edn/read

   Note that the function signature of select-nrepl.clojure.tools.reader/read and
   select-nrepl.clojure.tools.reader.edn/read is not the same for eof-handling"
...
(defn read-string
   ...
   ***WARNING***
   ...
   To read data structures only, use select-nrepl.clojure.tools.reader.edn/read-string

   Note that the function signature of select-nrepl.clojure.tools.reader/read-string and
   select-nrepl.clojure.tools.reader.edn/read-string is not the same for eof-handling"
  ([s]
   (read-string {} s))
  ([opts s]
